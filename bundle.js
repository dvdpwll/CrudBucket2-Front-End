...
/***/ function(module, exports, __webpack_require__) {
	...
	var createRootFolder = function createRootFolder(data) {
	  var folderData = {
	    folder: {
	  ...
	  // send data
	  api.createRootFolder(folderData).done(ui.success).fail(ui.onError);
	};

	// sign up a new user
	var onSignUp = function onSignUp(event) {
	  var data = getFormFields(event.target); //convert this to target text field
	  event.preventDefault(); //wont need this when I get rid of submit button

	  //send form data
	  api.signUp(data).done(createRootFolder).fail(ui.failure);

	  $('#sign-up').modal('hide');
	};

	// display user folders
	var displayUserFolder = function displayUserFolder(data) {
	  //make createdat and updatedat look better
	  for (var i = 0; i < data.folders.length; i++) {
	    //split date-time string into date and timef
	    var strSplitC = data.folders[i].createdAt.split('T');
	    var strSplitU = data.folders[i].updatedAt.split('T');
	  ...
	  //handlebars
	  var userFolderTemplate = __webpack_require__(8);
	...
	// display user files
	var displayUserFile = function displayUserFile(data) {
	  //make createdat and updatedat look better
	  for (var i = 0; i < data.files.length; i++) {
	    //split date-time string into date and timef
	    var strSplitC = data.files[i].createdAt.split('T');
	    var strSplitU = data.files[i].updatedAt.split('T');
	  ...
	  //handlebars
	  var userFileTemplate = __webpack_require__(29);
	...
	var getRootContents = function getRootContents(data) {
	  //update current folder path
	  app.currentPath = app.currentPath + ',' + data.folders[0]._id;

	  // get current path to show contents of root
	  var search = app.currentPath;

	  // send data to folders
	  api.showRootFolder(search).done(displayUserFolder).fail(ui.onError);

	  // send data to files
	  api.showRootFiles(search).done(displayUserFile).fail(ui.onError);
	};

	// after sign in, get the root folder
	var getRootFolder = function getRootFolder(data) {
	  //save token
	  app.user = data.user;
	  ...
	  // send data
	  api.showRootFolder(search).done(getRootContents).fail(ui.onError);
	};

	// display all the users
	var displayUsers = function displayUsers(data) {
	  //handlebars
	  var userTemplate = __webpack_require__(31);
	...
	var onGetUsers = function onGetUsers() {
	  //send index
	  api.getUsers().done(displayUsers).fail(ui.onError);
	};

	// sign in a user
	var onSignIn = function onSignIn(event) {
	  var data = getFormFields(this); //convert this to target text field
	  event.preventDefault(); //wont need this when I get rid of submit button

	  // send form data
	  api.signIn(data).done(getRootFolder).done(onGetUsers).done(ui.signInSuccess).fail(ui.failure);
	...
	// add folder to UI
	var addOneFolder = function addOneFolder(data) {
	  //split date-time string into date and timef
	  var strSplitC = data.folder.createdAt.split('T');
	  var strSplitU = data.folder.updatedAt.split('T');
	  ...
	  //handlebars
	  var userFolderTemplate = __webpack_require__(28);
	...
	var onCreateFolder = function onCreateFolder(event) {
	  var folderData = getFormFields(this); //convert this to target text field
	  event.preventDefault(); //wont need this when I get rid of submit button
	  ...
	  // send form data
	  api.createFolder(data).done(addOneFolder).fail(ui.onError);
	  $('#create-folder').modal('hide');
	};

	// add file to UI
	var addOneFile = function addOneFile(data) {
	  //split date-time string into date and timef
	  var strSplitC = data.file.createdAt.split('T');
	  var strSplitU = data.file.updatedAt.split('T');
	  ...
	  //handlebars
	  var userFileTemplate = __webpack_require__(30);
	  $('#main-content').append(userFileTemplate({
	    name: data.file.name,
	    _id: data.file._id,
	    url: data.file.url,
	...
	var openFolder = function openFolder(newPath) {
	  // update current path
	...
	var displayOtherUserFolder = function displayOtherUserFolder(data) {
	  //make createdat and updatedat look better
	  for (var i = 0; i < data.folders.length; i++) {
	    //split date-time string into date and timef
	    var strSplitC = data.folders[i].createdAt.split('T');
	  ...
	  //handlebars
	  var otherUserFolderTemplate = __webpack_require__(32);
	...
	var displayOtherUserFile = function displayOtherUserFile(data) {
	  //make createdat and updatedat look better
	  for (var i = 0; i < data.files.length; i++) {
	    //split date-time string into date and timef
	    var strSplitC = data.files[i].createdAt.split('T');
	  ...
	  //handlebars
	  var otherUserFileTemplate = __webpack_require__(33);
	...
	var getOtherRootContents = function getOtherRootContents(data) {
	  // update current path
	  app.currentPath = app.currentPath + ',' + data.folders[0]._id;
	  ...
	  // show folders
	  api.showRootFolder(search).done(displayOtherUserFolder).fail(ui.onError);

	  // show files
	  api.showRootFiles(search).done(displayOtherUserFile).fail(ui.onError);
	};

	// user clicks on other users folder
	var openOtherFolder = function openOtherFolder(newPath) {
	  // update current path
	  app.currentPath = newPath;
	...
	var onIcon = function onIcon(event) {
	  ...
	  if (target.hasClass('rename-button')) {
	    ...
	    // send data and Id
	    api.renameFile(data, fileId).done($(target).prevAll('h5:last').text(newName)).fail(ui.onError);
	  }
	  // delete file
	  else if (target.hasClass('delete-button')) {
	      // get file id
	      var _fileId = target.data('fileId');

	      // send id
	      api.deleteFile(_fileId).done($(target).parent().remove()).fail(ui.onError);
	    }
	    // rename folder
	        ...
	        // send data and id
	        api.renameFolder(_data, folderId).done($(target).prevAll('h5:last').text(_newName)).fail(ui.onError);
	      }
	      // delete folder
	      else if (target.hasClass('delete-folder-button')) {
	          // get folder id
	          var _folderId = target.data('folder-id');

	          // send id
	          api.deleteFolder(_folderId).done($(target).parent().remove()).fail(ui.onError);
	        }
	        // open folder
	            ...
	            // get folder id
	            var _folderId2 = target.nextAll('button:first').data('folder-id');

	            // set path for search
	            var newPath = path + ',' + _folderId2;
	          ...
	          // open other users folder
	              ...
	              // get path
	              var _path = target.data('path');

	              // get folder id
	              var _folderId3 = target.data('folder-id');

	              // set path for search
	              var _newPath = _path + ',' + _folderId3;

	              // open folder
	              openOtherFolder(_newPath);
	            }
	};

	// handlers for users
	var onUser = function onUser(event) {
	  ...
	  // if user clicks on any other user
	      ...
	      // get path for search
	      var search = target.data('path');

	      // update current path
	      app.currentPath = search;

	      // open other user root
	      api.showRootFolder(search).done(getOtherRootContents).fail(ui.onError);
	    }
	};
	...
	var uploadFile = function uploadFile(event) {
	  ...
	  api.uploadAWS(data).done(addOneFile).done($('#upload-file').modal('hide')).fail(function (err) {
	    return console.error(err);
	  });
	};

	// go up one folder level
	var upLevel = function upLevel() {
	  //get current path
	  var path = app.currentPath;

	  //split then reassemble w/o last segment of path
	  var strSplit = path.split(',');
	  var newPath = '';
...
/***/ function(module, exports, __webpack_require__) {
	...
	// get root files
	var showRootFiles = function showRootFiles(path) {
	  return $.ajax({
	...
	// get root folder
	var showRootFolder = function showRootFolder(path) {
	  return $.ajax({
	...
	// rename file
	var renameFile = function renameFile(data, fileId) {
	  return $.ajax({
	...
	// rename a folder
	var renameFolder = function renameFolder(data, folderId) {
	  return $.ajax({
	...
	var uploadAWS = function uploadAWS(data) {
	  return $.ajax({
	    url: app.api + '/files',
	    method: 'POST',
	    headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
	  ...
	  return "\n"
	    + ((stack1 = container.invokePartial(__webpack_require__(28),depth0,{"name":"current-user-folder","data":data,"helpers":helpers,"partials":partials,"decorators":container.decorators})) != null ? stack1 : "")
	    + "\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	    var stack1;

	  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.folders : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<div class=\"col-xs-4 col-sm-2 folder-info\">\n  <img class=\"icon foldericon\" src=\"icon-folder.png\" data-path=\""
	    ...
	    + "</h6>\n  <input type=\"text\" class=\"rename-folder-input\" name=\"New Folder Name\" placeholder=\"New Folder Name\">\n  <button type=\"button\" class=\"btn btn-warning btn-xs rename-folder-button\" data-folder-id=\""
	    + alias4(((helper = (helper = helpers._id || (depth0 != null ? depth0._id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"_id","hash":{},"data":data}) : helper)))
	    + "\">Rename</button>\n  <button type=\"button\" class=\"btn btn-danger btn-xs delete-folder-button\" data-folder-id=\""
	    + alias4(((helper = (helper = helpers._id || (depth0 != null ? depth0._id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"_id","hash":{},"data":data}) : helper)))
	    + "\">Delete</button>\n</div>\n";
	},"useData":true});
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
	  ...
	  return "\n"
	    + ((stack1 = container.invokePartial(__webpack_require__(30),depth0,{"name":"current-user-file","data":data,"helpers":helpers,"partials":partials,"decorators":container.decorators})) != null ? stack1 : "")
	    + "\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	    var stack1;

	  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.files : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
	},"usePartial":true,"useData":true});
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<div class=\"col-xs-4 col-sm-2 folder-info\">\n  <a href=\""
	    ...
	    + "\" target=\"_blank\">\n  <img class=\"icon\" src=\"icon-file.png\">\n  </a>\n  <h5>"
	    + alias4(((helper = (helper = helpers.name || (depth0 != null ? depth0.name : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"name","hash":{},"data":data}) : helper)))
	    + "</h5>\n  <h6>Created: "
	    + alias4(((helper = (helper = helpers.createdAt || (depth0 != null ? depth0.createdAt : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"createdAt","hash":{},"data":data}) : helper)))
	    + "</h6>\n  <h6>Modified: "
	    + alias4(((helper = (helper = helpers.updatedAt || (depth0 != null ? depth0.updatedAt : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"updatedAt","hash":{},"data":data}) : helper)))
	    + "</h6>\n  <input type=\"text\" class=\"rename-file-input\" name=\"New Filename\" placeholder=\"New Filename\">\n  <button type=\"button\" class=\"btn btn-warning btn-xs rename-button\" data-file-id=\""
	    + alias4(((helper = (helper = helpers._id || (depth0 != null ? depth0._id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"_id","hash":{},"data":data}) : helper)))
	    + "\">Rename</button>\n  <button type=\"button\" class=\"btn btn-danger btn-xs delete-button\" data-file-id=\""
	    + alias4(((helper = (helper = helpers._id || (depth0 != null ? depth0._id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"_id","hash":{},"data":data}) : helper)))
	    + "\">Delete</button>\n</div>\n";
	},"useData":true});
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
	  ...
	  return "\n  <li><a class=\"username\" href=\"#\" data-path=\","
	    + alias4(((helper = (helper = helpers._id || (depth0 != null ? depth0._id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"_id","hash":{},"data":data}) : helper)))
	    + "\" data-id=\""
	    + alias4(((helper = (helper = helpers._id || (depth0 != null ? depth0._id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"_id","hash":{},"data":data}) : helper)))
	    + "\">"
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
	  ...
	  return "\n<div class=\"col-xs-4 col-sm-2 folder-info\">\n  <img class=\"icon otherfoldericon\" src=\"icon-folder.png\" data-path=\""
	    + alias4(((helper = (helper = helpers.path || (depth0 != null ? depth0.path : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"path","hash":{},"data":data}) : helper)))
	    + "\" data-folder-id=\""
	    + alias4(((helper = (helper = helpers._id || (depth0 != null ? depth0._id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"_id","hash":{},"data":data}) : helper)))
	    + "\">\n  <h5>"
